...
library(fpc)
library(BurStMisc)
# IMPORTAR DATOS (algunas fechas tienen errores, ninguno importante porque solo interesa F.Baja y F.Alta que están bien)
...
for (i in 1:(ncol(xe)-1)){ 
  ...
  plot(xr$perpc,xr$perimp, ylab="Percentil Impacto(Yimp)",xlab="Percentil Probabilidad Cancelación (Yc)", col=col,pch=16,main=names(xe)[i], xlim=c(0,100), ylim=c(0,100))
...
# Calcular los el límite superior de las variables activas
